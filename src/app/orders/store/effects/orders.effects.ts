...
export class OrdersEffects {
   ...
   */
  @Effect()
  loadOpenOrders$: Observable<Action> = this.actions$.pipe(ofType<ordersActions.LoadOpenOrdersAction>(ordersActions.LOAD_OPEN_ORDERS)).pipe(
    switchMap(action => {
      return this.ordersService.getOpenOrders(action.payload).pipe(
        map(orders => {
          if (action.payload.concat) {
            return new ordersActions.SetMoreOpenOrdersAction({
              openOrders: orders.items,
              count: orders.count,
            });
          }
          return new ordersActions.SetOpenOrdersAction({
            openOrders: orders.items,
            count: orders.count,
          });
        }),
  ...
  loadHistoryOrders$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(action => {
        return this.ordersService.getClosedOrders(action.payload).pipe(
          map(orders => {
            if (action.payload.concat) {
              return new ordersActions.SetMoreHistoryOrdersAction({
                historyOrders: orders.items,
                count: orders.count,
              });
            }
            return new ordersActions.SetHistoryOrdersAction({
              historyOrders: orders.items,
              count: orders.count,
            });
          }),
  ...
  loadLastHistoryOrders$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(action => {
        return this.ordersService.getLastClosedOrders(action.payload).pipe(
          map(orders => {
            return new ordersActions.SetHistoryOrdersAction({
              historyOrders: orders.items,
              count: orders.count,
  ...
  cancelOpenOrder$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(([action, activePair]) => {
        return this.ordersService.deleteOrder(action.payload.order).pipe(
          map(() => {
