...
export class CoreEffects {
  ...
  loadSimpleCurrencyPairs$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(list => {
        if (list.length) {
          return of(
            new coreActions.SetCurrencyPairsAction({
              simpleItems: list.map(({ id, name }) => ({ id, name })),
              detailedItems: list,
            })
          );
        }
        return this.coreService.getSimpleCurrencyPairs().pipe(
          map(
            pairs =>
              new coreActions.SetCurrencyPairsAction({
  ...
  loadAllCurrencies$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(list => {
        ...
        return this.coreService.getCryptoFiatNames().pipe(
  ...
  loadCryptoCurrencies$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(list => {
        ...
        return this.coreService.getCryptoNames().pipe(
  ...
  loadFiatCurrencies$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(list => {
        ...
        return this.coreService.getFiatNames().pipe(
  ...
  loadVerificationStatus$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(() => {
        return this.settingsService.getCurrentVerificationStatusKYC().pipe(
