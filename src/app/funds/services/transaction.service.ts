...
export class TransactionsService {
  ...
  getLastTransactionsHistory({ offset, limit }): Observable<ResponseModel<TransactionHistoryItem[]>> {
    const params = {
      limit: limit + '',
      offset: offset + '',
    };
    return this.http.get<ResponseModel<TransactionHistoryItem[]>>(`${this.apiUrl}/api/private/v2/balances/inputOutputData/default`, {
      params,
    });
  }

  // request to get closed orders
  downloadExcel({ currencyId, currencyName, dateFrom, dateTo }): Observable<any> {
    const params = {
      currencyId,
      currencyName,
    };
