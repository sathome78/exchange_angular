...
export class FundsEffects {
  ...
  loadMaxCurrencyPair$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(action => {
        return this.balanceService.getMaxCurrencyPairByName(action.payload).pipe(
  ...
  loadPendingRequests$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(action => {
        return this.balanceService.getPendingRequests(action.payload).pipe(
          map(bal => {
            if (action.payload.concat) {
              return new fundsActions.SetMorePendingReqAction({
                items: bal.items,
                count: bal.count,
              });
            }
            return new fundsActions.SetPendingReqAction({
              items: bal.items,
              count: bal.count,
            });
          }),
  ...
  loadQuberaKycStatus$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(() => {
        return this.balanceService.getStatusKYC().pipe(
  ...
  loadBalanceDet$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(action => {
        return this.balanceService.getBalanceDetailsInfo(action.payload).pipe(
  ...
  RevokePendingRequests$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(action => {
        return this.balanceService.revokePendingRequest(action.payload.revoke).pipe(
  ...
  loadHistoryTransactions$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(action => {
        return this.transactionsService.getTransactionsHistory(action.payload).pipe(
          map(orders => {
            if (action.payload.concat) {
              return new fundsActions.SetMoreTransactionsHistoryAction({
                items: orders.items,
                count: orders.count,
              });
            }
            return new fundsActions.SetTransactionsHistoryAction({
              items: orders.items,
              count: orders.count,
            });
          }),
  ...
  loadLastHistoryTransactions$: Observable<Action> = this.actions$
    ...
    .pipe(
      switchMap(action => {
        return this.transactionsService.getLastTransactionsHistory(action.payload).pipe(
          map(
            orders =>
              new fundsActions.SetTransactionsHistoryAction({
                items: orders.items,
                count: orders.count,
              })
          ),
