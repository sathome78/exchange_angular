...
export class DashboardEffects {
  ...
  loadOpenOrdersDashboard$: Observable<Action> = this.actions$
    ...
    .pipe(switchMap((action) => {
      return this.ordersService.getOpenOrders(action.payload)
        .pipe(
          map(orders => {
            return new dashboardActions.SetOpenOrdersAction({openOrders: orders.items, count: orders.count});
          }),
  ...
  loadHistoryOrdersDashboard$: Observable<Action> = this.actions$
    ...
    .pipe(switchMap((action) => {
      return this.ordersService.getHistory(action.payload, 'CLOSED')
        .pipe(
          map((orders) => {
            return new dashboardActions.SetHistoryOrdersAction({historyOrders: orders.items, count: orders.count});
          }),
